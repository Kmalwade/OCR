...
from .tasks import process_ocr_extraction
...
class OCR_Extraction(APIView):
    def post(self, request):
        try:
            ...
            process_ocr_extraction.delay(result)
            return Response(res, status=status.HTTP_200_OK)

        except Exception as ex:
            print(ex)
            res = {"Satus": False, "msg": "Internal server error"}
            return JsonResponse(res, status=status.HTTP_503_SERVICE_UNAVAILABLE)


class StoreIntoDB(APIView):
    def post(self, request):
        try:
            name = request.data.get('name', '')
            data = request.data.get('data', {})
            my_model = MyModel(name=name, data=data)
            my_model.save()
